...
DATA = ROOT / "data" / "n4-data.js"
REPORT_DIR = ROOT / "reports" / "n4-repair"
...
def audit_story(story: dict, root: Path = ROOT, audio_base: str = "assets/n4/audio") -> list[dict]:
    found: list[dict] = []
    ...
    for row_index, row in enumerate(vocab, start=1):
        ...
        if not number.isdigit() or int(number) < 1 or int(number) > 1035:
...
def audit_data(data: dict, root: Path = ROOT) -> dict:
    ...
    if len(stories) != 262:
        issues.append(issue("阻断发布", "STORY_COUNT", f"N4 应为 262 条，实际 {len(stories)} 条"))
    if ids != list(range(1, 263)) or len(ids) != len(set(ids)):
        issues.append(issue("阻断发布", "STORY_IDS", "story id 不连续、乱序或重复"))
    topic_ids = [story.get("topicId") for story in stories]
    if any(not isinstance(value, int) or not 1 <= value <= 13 for value in topic_ids):
        issues.append(issue("阻断发布", "TOPIC_ID_INVALID", "存在超出 1–13 的 topicId"))
    ...
    for story in stories:
        story_issues = audit_story(story, root, data.get("audioBase", "assets/n4/audio"))
...
def write_reports(result: dict) -> None:
    ...
    (REPORT_DIR / "n4-audit.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lines = ["# N4 全量内容审计", "", f"- 故事数：{result['story_count']}", f"- 已校对：{result['reviewed_count']}", f"- 问题总数：{result['issue_count']}", ""]
    for severity in SEVERITIES:
        ...
        lines.append("")
    (REPORT_DIR / "n4-audit.md").write_text("\n".join(lines), encoding="utf-8")

    summary = ["# N4 审计摘要", "", f"- N4 总条数：{result['story_count']}（要求 262）", f"- 已校对条数：{result['reviewed_count']}", f"- 检出问题：{result['issue_count']}", ""]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="审计 N4 全部 262 条内容数据")
