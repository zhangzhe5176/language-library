...
DATA = ROOT / "data" / "n2-data.js"
REPORT_DIR = ROOT / "reports" / "n2-repair"
...
def suspicious_chinese(text: str) -> bool:
    ...
    for acronym in ("SNS", "WEB", "APP", "INS", "AI", "IT", "MB", "PC"):
        scrubbed = scrubbed.replace(acronym, "")
    scrubbed = re.sub(r"\bism\b", "", scrubbed)
    scrubbed = re.sub(r"\b(AI|SNS|WEB|IT|MB|APP|PC|C)\b", "", scrubbed)
    scrubbed = scrubbed.replace(".com", "")
    return bool(re.search(r"[A-Za-z]{3,}", scrubbed))


def audit_story(story: dict, root: Path = ROOT, audio_base: str = "assets/n2/audio") -> list[dict]:
    found: list[dict] = []
    ...
    if story.get("type") == "dialogue" and japanese and any(line.get("speaker") for line in japanese) and (len(speakers) < 2 or any(not line.get("speaker") for line in japanese)):
    ...
    for row_index, row in enumerate(vocab, start=1):
        ...
        if not number.isdigit() or int(number) < 1 or int(number) > 2360:
        ...
        if story.get("reviewStatus") == "已校对" and (pos not in ALLOWED_POS or re.search(r"[A-Za-z]", pos)):
            found.append(issue("字段异常", "VOCAB_POS_INVALID", f"词性不符合 N3/N4/N5 中文缩写风格：{pos}", story, "vocab"))
...
def audit_data(data: dict, root: Path = ROOT) -> dict:
    ...
    if len(stories) != 452:
        issues.append(issue("阻断发布", "STORY_COUNT", f"N2 应为 452 条，实际 {len(stories)} 条"))
    if ids != list(range(1, 453)) or len(ids) != len(set(ids)):
        issues.append(issue("阻断发布", "STORY_IDS", "story id 不连续、乱序或重复"))
    topic_ids = [story.get("topicId") for story in stories]
    if any(not isinstance(value, int) or not 1 <= value <= 23 for value in topic_ids):
        issues.append(issue("阻断发布", "TOPIC_ID_INVALID", "存在超出 1–23 的 topicId"))
    ...
    for story in stories:
        story_issues = audit_story(story, root, data.get("audioBase", "assets/n2/audio"))
...
def write_reports(result: dict) -> None:
    ...
    (REPORT_DIR / "n2-audit.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lines = ["# N2 全量内容审计", "", f"- 故事数：{result['story_count']}", f"- 已校对：{result['reviewed_count']}", f"- 问题总数：{result['issue_count']}", ""]
    for severity in SEVERITIES:
        ...
        lines.append("")
    (REPORT_DIR / "n2-audit.md").write_text("\n".join(lines), encoding="utf-8")

    summary = ["# N2 审计摘要", "", f"- N2 总条数：{result['story_count']}（要求 452）", f"- 已校对条数：{result['reviewed_count']}", f"- 检出问题：{result['issue_count']}", ""]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="审计 N2 全部 452 条内容数据")
