...
- 页面主标题：`clamp(2.35rem, 6vw, 5.25rem)`，紧凑行高，适合门户和等级首页。
- 区块标题：`clamp(1.55rem, 3vw, 2.4rem)`。
- 卡片标题：约 `1.08rem`，配合等级、Topic 或语言标签。
- 日文正文：使用 serif 类日文字体，`clamp(1.45rem, 3vw, 2.25rem)`，行高约 `1.75`。
- 辅助文字：低对比灰色，行高约 `1.78`。
...
- 页面最大宽度：`1180px`。
- 桌面端页面外边距：`26px–48px`。
- 手机端页面外边距：`14px`，320px 下压缩到 `10px`。
- 卡片间距：`18px`。
- 主要布局：
  - 门户和等级首页：大 Hero + 卡片网格。
  - 学习页：桌面端主内容 + 右侧状态栏，移动端单列。
...
- 1024px：语言/等级卡片降为三列，Topic 降为两列，学习页可保持或接近单列。
- 768px：核心内容进入单列，导航保留横向滚动。
...
- 检查视口：1440px、1024px、768px、390px、320px。
